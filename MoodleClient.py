...
class MoodleClient(object):
    def __init__(self, user, passw, host='', repo_id=4, proxy: ProxyCloud = None):
        ...
        # HEADERS MEJORADOS PARA SIMULAR CONEXIÓN DESDE CUBA
        self.baseheaders = self.get_cuban_headers()
        
    def get_cuban_headers(self):
        """Genera headers que simulan conexión desde Cuba"""
        user_agents = [
        ...
        cuban_ips = [
            '190.6.224.42', '200.55.182.126', '190.6.227.58',
            '190.6.225.42', '190.6.226.58', '190.6.228.42'
        ]
        ...
        return {
            ...
            # Headers específicos para simular Cuba
            'X-Forwarded-For': random.choice(cuban_ips),
            'X-Real-IP': random.choice(cuban_ips),
            'CF-IPCountry': 'CU',
            'X-Client-GeoLocation': 'CU',
            'X-Client-Region': 'America/Havana',
        }

    def getsession(self):
        return self.session

    def pre_connect_cuban_sites(self):
        """Hace peticiones preliminares a sitios cubanos para establecer contexto"""
        cuban_sites = [
            'https://www.etecsa.cu/',
        ...
        for site in cuban_sites:
            try:
                test_headers = {
                    **self.baseheaders,
                    'Referer': 'https://www.google.cu/',
                    'Origin': 'https://www.google.cu'
                }
                self.session.get(site, headers=test_headers, proxies=self.proxy, timeout=5)
                time.sleep(0.5)  # Pequeña pausa entre peticiones
            except:
                continue  # Ignorar errores, es solo para establecer contexto
    ...
    def getUserData(self):
        try:
            ...
            # Headers específicos para token con referer cubano
            token_headers = {
                **self.baseheaders,
                'Referer': self.path,
                'Origin': self.path.rstrip('/'),
                'X-Requested-With': 'XMLHttpRequest'
            }
            
            resp = self.session.get(tokenUrl, proxies=self.proxy, headers=token_headers)
    ...
    def login(self):
        try:
            # Pre-conexión a sitios cubanos para establecer contexto
            self.pre_connect_cuban_sites()
            
            login = self.path + 'login/index.php'
            
            # Primera petición al login con headers mejorados
            login_headers = {
                **self.baseheaders,
                'Referer': 'https://www.google.cu/',
                'Origin': 'https://www.google.cu'
            ...
            resp = self.session.get(login, proxies=self.proxy, headers=login_headers)
            ...
            resp2 = self.session.post(loginurl, data=payload, proxies=self.proxy, headers=post_headers)
